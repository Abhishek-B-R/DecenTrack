...
const SIM_BASE_URL = process.env.SIM_BASE_URL || "http://localhost:8000";
const wss = new WebSocketServer({ port: 8080 });
let allSocket: WebSocket[] = [];
...
async function fetchWebsites() {
  const res = await axios.get(`${SIM_BASE_URL}/websites`);
  // Backend returns { websites: [{ id, url, ... }] }
  const websites = res.data.websites || [];
  // Match your old finalData shape
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  return websites.map((w: any) => ({ id: w.id, url: w.url }));
}

setInterval(async () => {
  try {
    const finalData = await fetchWebsites();
    const payload = JSON.stringify({ type: "websiteUpdate", data: finalData });
    allSocket.forEach((socket) => {
      if (socket.readyState === WebSocket.OPEN) socket.send(payload);
    });
  } catch (e) {
    console.error("Error fetching websites from simulator:", e);
  }
}, 300000); // 5 mins

console.log("Hub WebSocket server listening on ws://localhost:8080");