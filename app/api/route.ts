...
// app/api/route.ts (or pages/api/index.ts)
export async function POST(req: Request) {
  const body = await req.json(); // { data: TickInput[] }
  const res = await fetch(`${process.env.SIM_BASE_URL}/tx/addMultipleTicks?validator=sim-validator`, {
    method: "POST",
