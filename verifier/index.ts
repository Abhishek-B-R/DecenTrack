...
import { sendLowBalanceEmail, sendWebsicyanertEmail } from "./email.ts";
import { sendLowBalanceSMS, sendWebsiteDownSMS } from "./sms.ts";
const WebsiteMonitorABI = tracking.abi;
const WebsiteMonitor = new ethers.Contract(
  ContractAddress,
  WebsiteMonitorABI,
  provider
);
...
setInterval(async () => {
  try {
    ...
    websiteIds.forEach(async (websiteId, index) => {
      const websiteTicks = await WebsiteMonitorSigned.getRecentTicks(
        websiteId,
        10
      );
      let downCount = 0;
      ...
      if (downCount > 5) {
        ...
        allContacts.forEach((contact: string) => {
          if (contact.includes("@")) {
            sendWebsicyanertEmail(contact, websiteDetails[index][0]);
          } else {
