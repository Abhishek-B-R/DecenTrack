...
// Inside `npx hardhat console --network sepolia`
const contractAddress = "0x2eF7E8bdB865ef8a2e790beE6DD0510B0A421B89";
const WebsiteMonitor = await ethers.getContractAt(
  "WebsiteMonitor",
  contractAddress
);

// Register a validator
await WebsiteMonitor.registerValidator(
  "0x1aB98C06b3FaB72a124E34d39aaCfe7F5a6094De",
  "Hubli"
);
console.log("Validator registered.");

// check if validator is authenticated or not
const isRegistered = await WebsiteMonitor.isValidatorAuthenticated(
  "0x1aB98C06b3FaB72a124E34d39aaCfe7F5a6094De"
);
console.log("Is validator authenticated?", isRegistered);

// get all the validator details
const validatorDetails = await WebsiteMonitor.getValidator(
  "0x1aB98C06b3FaB72a124E34d39aaCfe7F5a6094De"
);
...
// Create a website
const tx = await WebsiteMonitor.createWebsite(
  "https://googler.com",
  "abhishekbr989@gmail.com"
);
...
data[3].forEach((tick) => {
  console.log(
    `- Validator: ${tick[0]}, Time: ${tick[1]}, Status: ${
      tick[2] == 0 ? "Good" : "Bad"
    }, Latency: ${tick[3]}`
  );
});
...
ticks.forEach((tick) => {
  console.log(
    `Validator: ${tick.validator}, Time: ${tick.createdAt}, Status: ${tick.status}, Latency: ${tick.latency}`
  );
});
...
recentTicks.forEach((tick) => {
  console.log(
    `Validator: ${tick.validator}, Time: ${tick.createdAt}, Status: ${tick.status}, Latency: ${tick.latency}, Location: ${tick.location}`
  );
});
...
await WebsiteMonitor.addWebsiteBalance(websiteId, {
